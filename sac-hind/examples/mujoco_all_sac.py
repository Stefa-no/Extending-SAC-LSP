...
from sac.misc.sampler import PrioritizedSampler
from sac.replay_buffers import PrioritizedReplayBuffer
...
ENVIRONMENTS = {
    ...
    'pong': {
        'default': lambda: normalize(GymEnv('Pong-v0'))
    },
}
...
def parse_args():
    ...
    parser.add_argument('--her_type', type=int, default=0)
...
def run_experiment(variant):
    ...
    env_copy = normalize(ENVIRONMENTS[domain][task](**env_params))

    pool = PrioritizedReplayBuffer(env_spec=env, **replay_buffer_params)

    sampler = PrioritizedSampler(her_type=variant['her_type'], goal_type=variant['her_goal'], **sampler_params)

    base_kwargs = dict(algorithm_params['base_kwargs'], sampler=sampler)

    M = value_fn_params['layer_size']
    qf = NNQFunction(env_spec=env, hidden_layer_sizes=(M, M))
    vf = NNVFunction(env_spec=env, hidden_layer_sizes=(M, M))
...
def main():
    ...
    variant_generator = get_variants(domain=domain, task=task, policy=args.policy, args=args)
