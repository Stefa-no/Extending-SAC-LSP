...
class MLPPreprocessor(MLPFunction):
    def __init__(self, env_spec, layer_sizes=(128, 16),
        ...
        goals_ph = tf.placeholder(
            tf.float32,
            shape=(None, self._Dg),
            name='goal',
        )

        self._input_pls = (obs_ph, goals_ph, )
        self._layer_sizes = layer_sizes
        self._output_nonlinearity = output_nonlinearity
        # since here there was no list, maybe better to concat before.
        self._output_t = self.get_output_for(*(obs_ph, goals_ph ), reuse=tf.AUTO_REUSE)
