...
class LatentSpacePolicy(NNPolicy, Serializable):
    ...
    def __init__(self,
        ...
        self._Ds = env_spec.spec.observation_space.flat_dim
    ...
    def actions_for(self, observations, goals, latents=None,
        ...
        with tf.variable_scope(name, reuse=reuse):
            conditions = (
                self._observations_preprocessor.get_output_for(
                   *(observations, goals), reuse=reuse)
    ...
    def build(self):
        ...
        self._raw_actions, self._actions, self._log_pis = self.actions_for(self._observations_ph, self._goals_ph, 
                                                    with_log_pis=True, with_raw_actions=True)
        self._determistic_actions = self.actions_for(self._observations_ph, self._goals_ph,
                                                     self._latents_ph)
    # should add goal here
    def get_action(self, observation, goal):
        ...
        if self._is_deterministic and self._n_map_action_candidates > 1:
            ...
            goals = np.tile(
                goal[None], reps=(self._n_map_action_candidates, 1))
            action_candidates = self.get_actions(observations, goals)
            # do I need to modify _q_function.eval as well? probably, but not sure how
            q_values = self._q_function.eval(*(observations,goals), action_candidates)
            best_action_index = np.argmax(q_values)

            return action_candidates[best_action_index], {}
        return self.get_actions(observation[None], goal[None])[0], {}

    # should add goals here
    def get_actions(self, observations, goals):
        """Sample batch of actions based on the observations"""

        feed_dict = { self._observations_ph: observations,
                      self._goals_ph: goals }
