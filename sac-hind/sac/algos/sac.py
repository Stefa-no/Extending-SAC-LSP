...
class SAC(RLAlgorithm, Serializable):
    ...
    def __init__(
        ...
        self._Dg = self._env._wrapped_env.model.nv
    ...
    def train(self):
        ...
        self._train(self._env, self._env_copy, self._policy, self._pool)
    ...
    def _init_critic_update(self):
        ...
        self._qf_t = self._qf.get_output_for(
            self._observations_ph, self._goals_ph, self._actions_ph, reuse=True)  # N

        with tf.variable_scope('target'):
            vf_next_target_t = self._vf.get_output_for(self._next_observations_ph, self._goals_ph)  # N
    ...
    def _init_actor_update(self):
        ...
        actions, log_pi = self._policy.actions_for(self._observations_ph, self._goals_ph,
                                                   with_log_pis=True)

        self._vf_t = self._vf.get_output_for(self._observations_ph, self._goals_ph, reuse=True)  # N
        ...
        log_target = self._qf.get_output_for(
            self._observations_ph, self._goals_ph, actions, reuse=True)  # N
