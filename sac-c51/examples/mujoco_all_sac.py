...
def launch_experiments(variant_generator, args):
    ...
    # seems like the only thing that change here are the seeds, 
    # therefore doing 5 experiments every time might not be stricly necessary
    for i, variant in enumerate(variants):
        ...
        run_sac_experiment(
            ...
            n_parallel=1,
            seed=run_params['seed'],			#changes at each variant
            terminate_machine=True,
            log_dir=args.log_dir,
            snapshot_mode=run_params['snapshot_mode'], #usually always same through variants
            snapshot_gap=run_params['snapshot_gap'],   #usually always same through variants
            sync_s3_pkl=run_params['sync_pkl'],        #usually always same through variants
        )
