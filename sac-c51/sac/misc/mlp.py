...
def mlp(inputs,
    ...
    # The idea is to check if the nonlinearity is softmax and in that 
    # case to add the last layer with 51 as size and see how it goes
    if output_nonlinearity is not None:
        if type(output_nonlinearity) == type (tf.nn.softmax):
            layer = affine(layer, obj.NB_ATOMS,
                           W_initializer=W_initializer,
                           b_initializer=b_initializer)
        layer = output_nonlinearity(layer)
        
    if squeeze_output and not keep_distrib:
        if type(output_nonlinearity) == type(tf.nn.softmax):
            layer = tf.reduce_sum(obj.z * layer, axis=1)     #when using c51, we turn the distribution into value Qval(s_t, A(s_t)) like in build_model() of D4PG
        else:
...
class MLPFunction(Parameterized, Serializable):
    ...
    def __init__(self, name, input_pls, hidden_layer_sizes,
        ...
        #I think this +[none] is used to differentiate it from the preprocessing and therefore squeeze the output?
    ...
    def get_output_for(self, *inputs, reuse=False, keep_distrib=False):
        with tf.variable_scope(self._name, reuse=tf.AUTO_REUSE): #was reuse=reuse before
            value_t = mlp(
                inputs=inputs,
                output_nonlinearity=self._output_nonlinearity,
                layer_sizes=self._layer_sizes,
                obj=self,
                keep_distrib=keep_distrib
            )  # N
