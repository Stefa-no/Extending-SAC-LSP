...
from sac.misc.sampler import PrioritizedSampler, SimpleSampler
from sac.replay_buffers import PrioritizedReplayBuffer, SimpleReplayBuffer
...
def run_experiment(variant):
    ...
    #environments = np.asarray([normalize(ENVIRONMENTS[domain][task](**env_params)) for i in range(8)])

    #pool = SimpleReplayBuffer(env_spec=env.spec, **replay_buffer_params)
    pool = PrioritizedReplayBuffer(env_spec=env.spec, **replay_buffer_params)

    #sampler = SimpleSampler(**sampler_params)
    sampler = PrioritizedSampler(**sampler_params)

    base_kwargs = dict(algorithm_params['base_kwargs'], 
                       sampler=sampler,
                       worker_hosts=variant['worker_hosts'],
                       ps_hosts=variant['ps_hosts'],
...
def launch_experiments(variant_generator, args):
  ...
  #  print('Launching {} experiments.'.format(num_experiments))
   ...
   #     print("Experiment: {}/{}".format(i, num_experiments))
        ...
        # modify variant since it seems to be the only param that is passed
        # to run_experiment
        variant['ps_hosts'] = args.ps_hosts
        variant['worker_hosts'] = args.worker_hosts
        variant['job_name'] = args.job_name
        variant['task_index'] = args.task_index
