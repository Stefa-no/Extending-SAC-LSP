...
class PrioritizedReplayBuffer(ReplayBuffer, Serializable):
    def __init__(self, env_spec, max_replay_buffer_size):
        ...
        self._action_dim = env_spec.action_space.flat_dim
        self._max_buffer_size = max_replay_buffer_size
        self._observation = np.zeros((self.max_replay_buffer_size,
                                       self._observation_dim))
        # It's a bit memory inefficient to save the observations twice,
        # but it makes the code *much* easier since you no longer have to
        # worry about termination conditions.
        self._next_obs = np.zeros((self.max_replay_buffer_size,
                                   self._observation_dim))
        self._action = np.zeros((self.max_replay_buffer_size, self._action_dim))
        self._reward = np.zeros(self.max_replay_buffer_size)
        # self._terminals[i] = a terminal was received at time i
        self._terminal = np.zeros(self.max_replay_buffer_size, dtype='uint8')
        self._top = 0
        self._size = 0
    ...
    def prioritized_batch(self, batch_size):
     ...
     #   print ("BBBBBBBBBBBB  " + str(b))
        s = np.random.uniform(a, b)
     #   print("SSSSSSSSSSSS    " + str(s))

        for i in range(batch_size):
            (data_idx[i], batch_idx[i], batch_priorities[i]) = self.get(s[i])
      #  print("DATA INDEX    " + str(data_idx))
       # print("BATCH INDEX    " + str(batch_idx))
        #print("PRIORITIES    " + str(batch_priorities))
        
        batch = dict(
            observation=self._observation[data_idx],
            action=self._action[data_idx],
            reward=self._reward[data_idx],
            terminal=self._terminal[data_idx],
            next_observation=self._next_obs[data_idx],
        )
    ...
    def add_sample(self, observation, action, reward, terminal,
        ...
        self._observation[self._top] = observation
        self._action[self._top] = action
        self._reward[self._top] = reward
        self._terminal[self._top] = terminal
        self._next_obs[self._top] = next_observation

        if value==None:
            value= self.max()
        # originally value could be self.max or an abs of the loss (see sampler.update)
        # since now I always use the loss, I make take the abs of value here.
        self.update(self._top + self.max_replay_buffer_size - 1, np.abs(value))	##this part is new, as it the calls _propagate
    ...
    def random_batch(self, batch_size):
        indices = np.random.randint(0, self._size, batch_size)
        return dict(
            observation=self._observation[indices],
            action=self._action[indices],
            reward=self._reward[indices],
            terminal=self._terminal[indices],
            next_observation=self._next_obs[indices],
        )
    ...
    def __getstate__(self):
        ...
        d.update(dict(
            o=self._observation.tobytes(),
            a=self._action.tobytes(),
            r=self._reward.tobytes(),
            t=self._terminal.tobytes(),
            no=self._next_ob.tobytes(),
            top=self._top,
            size=self._size,
        ))
        return d
    ...
    def __setstate__(self, d):
        ...
        self._observation = np.fromstring(d['o']).reshape(
            self._max_buffer_size, -1
        )
        self._next_obs = np.fromstring(d['no']).reshape(
            self._max_buffer_size, -1
        )
        self._action = np.fromstring(d['a']).reshape(self._max_buffer_size, -1)
        self._reward = np.fromstring(d['r']).reshape(self._max_buffer_size)
        self._terminal = np.fromstring(d['t'], dtype=np.uint8)
        self._top = d['top']
        self._size = d['size']
